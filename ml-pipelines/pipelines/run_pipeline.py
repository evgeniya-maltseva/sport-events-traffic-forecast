...
from pipelines._utils import get_pipeline_driver, convert_struct, get_pipeline_args
from pipelines.project import get_project, get_project_tags
...
def main():  # pragma: no cover
    ...
    kwargs = get_pipeline_args()
    tags = convert_struct(args.tags)

    try:
        print("###### Creating/updating a SageMaker Project with the following definition:")
        project = get_project(**kwargs)
        tags.extend(get_project_tags(project))
        print(project)

        print("\n###### Creating/updating a SageMaker Pipeline with the following definition:")
        pipeline = get_pipeline_driver(args.module_name, kwargs)
